...
export default {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.post('http://localhost:8181/booksrc/updateAllById',this.ruleForm).then(function(resp){
            // if(resp.data === 'success'){
            //   _this.$alert('《'+_this.ruleForm.bookname+'》修改成功！', '消息', {
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8181/booksrc/findById/'+this.$route.query.id).then(function(resp){
